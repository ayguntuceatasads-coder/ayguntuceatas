...
export default function HomePage() {
  ...
  const [data, setData] = useState<any>({ settings: null, services: [], posts: [] });
  ...
  useEffect(() => {
    async function fetchAllData() {
      try {
        ...
        console.error("Beklenmedik bağlantı hatası:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  // Manuel Ok Kontrolleri (Sonsuz Döngü)
  const nextSlide = () => setCurrentSlide((prev) => (prev + 1) % slides.length);
  const prevSlide = () => setCurrentSlide((prev) => (prev - 1 + slides.length) % slides.length);

  // Otomatik Slayt Döngüsü (Manuel tıklandığında süreyi sıfırlar)
  useEffect(() => {
  ...
  }, [slides.length, currentSlide]); // currentSlide değiştiğinde (örneğin oka basıldığında) timer sıfırlanır

  if (loading) return (
    <div className="min-h-screen bg-[#082b34] flex items-center justify-center">
      <div className="w-12 h-12 border-4 border-[#6ec9c9] border-t-transparent rounded-full animate-spin"></div>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-slate-50 overflow-hidden">
      ...
      {/* 1. HERO SECTION - TAM EKRAN PREMİUM ALAN & ZARİF OKLAR */}
      <section className="relative w-full h-screen bg-[#061d24] overflow-hidden group">
        ...
        {/* ZARİF SAĞ - SOL YÖNLENDİRME OKLARI */}
        {slides.length > 1 && (
          <>
            <button onClick={prevSlide} className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 z-30 p-3 bg-white/10 hover:bg-white/30 backdrop-blur-md border border-white/20 rounded-full text-white transition-all opacity-0 group-hover:opacity-100 hover:scale-110 shadow-xl">
              <ChevronLeft className="w-6 h-6 md:w-8 md:h-8" />
            </button>
            <button onClick={nextSlide} className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 z-30 p-3 bg-white/10 hover:bg-white/30 backdrop-blur-md border border-white/20 rounded-full text-white transition-all opacity-0 group-hover:opacity-100 hover:scale-110 shadow-xl">
            ...
            <div className="absolute bottom-12 left-0 right-0 z-30 flex justify-center gap-3">
              {slides.map((_slide: any, i: number) => (
                <button 
                  ...
                  className={`h-1.5 transition-all duration-500 rounded-full ${i === currentSlide ? "bg-[#6ec9c9] w-12" : "bg-white/30 w-4 hover:bg-white/60"}`}
                ></button>
              ))}
            </div>
          </>
          ...
          {data.services.map((service: any) => (
            <div key={service.id} className="min-w-[85vw] md:min-w-[calc(33.333%-1rem)] snap-start bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden group hover:shadow-xl transition-all flex flex-col">
              <div className="h-56 overflow-hidden relative">
                <img src={service.image_url || "https://images.unsplash.com/photo-1544027993-37db48d5f06d"} alt={service.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
              </div>
      ...
      {/* 4. HAKKIMIZDA ÖZETİ (Görsel Panele Bağlandı) */}
      <section className="py-24 px-4 md:px-8 max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
        <div className="w-full lg:w-1/2 relative">
          ...
          {/* Admin panelden eklenen görsel yoksa varsayılan resim gelir */}
          <img src={data.settings?.about_image || "https://images.unsplash.com/photo-1559839734-2b71ea197ec2?q=80&w=1000"} alt="Hakkımızda Özet" className="rounded-[3rem] shadow-xl w-full object-cover aspect-[4/3]" />
        </div>
      ...
      {/* 5. ŞIK İLETİŞİM & RANDEVU AKSİYON BARI (CTA) */}
